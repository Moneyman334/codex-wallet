...
export default function SwapPage() {
  ...
  // Check if DEX aggregator API is available
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="shadow-xl">
          ...
          <CardContent className="space-y-4">
            ...
            {/* API Warning */}
            {quoteError && (quoteError as Error)?.message?.includes("API key") && (
              <div className="flex items-start gap-2 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                <AlertCircle className="h-4 w-4 text-yellow-500 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-yellow-600 dark:text-yellow-400">
                  DEX trading disabled: Aggregator API key required. Using fallback pricing for quotes.
                </p>
              </div>
            )}
            ...
            <div className="flex items-start gap-2 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
              ...
              <p className="text-sm text-blue-600 dark:text-blue-400">
                Aggregating 300+ DEX sources for best execution across {SUPPORTED_CHAINS.length} chains
              </p>
            </div>
          </CardContent>
        </Card>
