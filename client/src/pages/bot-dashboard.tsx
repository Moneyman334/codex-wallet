...
export default function BotDashboard() {
  ...
  const { data: unclaimedProfits, isLoading: profitsLoading } = useQuery({
    queryKey: ['/api/bot/profits/unclaimed'],
  });

  const claimProfitsMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest('/api/bot/profits/claim', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
    },
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ['/api/bot/profits/unclaimed'] });
      queryClient.invalidateQueries({ queryKey: ['/api/bot/trades', mockUserId] });
      
      toast({
        title: "Profits Claimed!",
        description: data.message || `Successfully claimed $${data.claimedAmount?.toFixed(2)}`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        {/* Unclaimed Profits Banner */}
        {!profitsLoading && unclaimedProfits && unclaimedProfits.totalProfit > 0 && (
          <Card className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-500/20" data-testid="card-unclaimed-profits">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground">Unclaimed Bot Profits</p>
                    <div className="flex items-center space-x-3 mt-1">
                      <h2 className="text-4xl font-bold text-green-500">
                        ${unclaimedProfits.totalProfit.toFixed(2)}
                      </h2>
                      <Badge variant="secondary" className="text-xs">
                        {unclaimedProfits.tradeCount} profitable {unclaimedProfits.tradeCount === 1 ? 'trade' : 'trades'}
                      </Badge>
                    </div>
                  </div>
                </div>
                <Button 
                  ...
                  onClick={() => claimProfitsMutation.mutate()}
                  disabled={claimProfitsMutation.isPending}
                  data-testid="button-claim-profits"
                >
