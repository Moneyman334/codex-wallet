...
const configSchema = z.object({
  exchangeApiKey: z.string().min(1, "API Key is required"),
  exchangeApiSecret: z.string().min(1, "API Secret is required"),
  exchangePassphrase: z.string().min(1, "Passphrase is required"),
...
export default function BotConfig() {
  ...
  const form = useForm<ConfigFormData>({
    ...
    defaultValues: {
      exchangeApiKey: (config as any)?.exchangeApiKey || '',
      exchangeApiSecret: (config as any)?.exchangeApiSecret || '',
      exchangePassphrase: (config as any)?.exchangePassphrase || '',
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: ConfigFormData) => {
      return await apiRequest('POST', '/api/bot/config', {
        ...
        exchangeApiKey: data.exchangeApiKey,
        exchangeApiSecret: data.exchangeApiSecret,
        exchangePassphrase: data.exchangePassphrase,
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Connect your cryptocurrency exchange account to enable automated trading. 
                  Refer to your exchange's documentation for API key setup.
                </CardDescription>
              </CardHeader>
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 flex items-start space-x-3">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-muted-foreground">
                      Your API keys are encrypted and stored securely. Enable only "Trade" permissions when creating your API keys. Never share your API secret.
                    </p>
                  </div>
                </div>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input 
                          type={showApiKeys ? "text" : "password"}
                          placeholder="Enter your Exchange Passphrase (if required)"
