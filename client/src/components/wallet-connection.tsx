...
export default function WalletConnection({
  ...
  const { mediumImpact, successNotification, warningNotification, errorNotification } = useHaptics();
  const { isEnabled: biometricEnabled, authenticate: biometricAuth, biometryTypeName } = useBiometric();
  const { shareWalletAddress, isSharing } = useShare();
  ...
  const handleShareAddress = async () => {
    if (account) {
      mediumImpact();
      const result = await shareWalletAddress(account);
      if (result) {
        successNotification();
        toast({
          title: "Shared",
  ...
  const handleSecureDisconnect = async () => {
    ...
    if (biometricEnabled) {
      const authenticated = await biometricAuth(`Authenticate with ${biometryTypeName} to disconnect wallet`);
      if (!authenticated) {
        errorNotification();
        toast({
          title: "Authentication Required",
          description: `${biometryTypeName} verification failed. Cannot disconnect.`,
          variant: "destructive",
        });
        return;
      }
        ...
        <CardContent className="p-6">
          ...
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground mb-6">
              Connect your wallet to access CODEX Wallet
            </p>
            <Button 
              onClick={onConnect}
              className="px-8"
