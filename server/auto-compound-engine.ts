...
import { REVENUE_CONFIG, logRevenueEvent, getRevenueWallet } from "./revenue-config";

class AutoCompoundEngine {
  private totalPlatformFeesCollected = 0;
  ...
  private async compoundStake(stake: any, pool: any) {
    ...
    const grossReward = currentBalance * periodRate;
    
    // Calculate platform performance fee (10% of rewards)
    const platformFeePercentage = REVENUE_CONFIG.stakingPoolFeePercentage / 100;
    const platformFee = grossReward * platformFeePercentage;
    const netReward = grossReward - platformFee;
    
    // Track platform fees collected
    this.totalPlatformFeesCollected += platformFee;

    // Calculate new balance after deducting platform fee
    const newBalance = currentBalance + netReward;

    // Update stake with net reward (after platform fee)
    const compoundCount = parseInt(stake.compoundCount || '0') + 1;
    const totalEarned = parseFloat(stake.totalEarned || '0') + netReward;
    ...
    // Create compound event (track gross reward and platform fee)
    await storage.createCompoundEvent({
      ...
      rewardAmount: netReward.toString(),
      apyAtCompound: pool.baseApy,
      platformFee: platformFee.toString()
    });
    
    // Record platform revenue from staking fee
    if (platformFee > 0) {
      try {
        await storage.recordPlatformRevenue({
          source: 'staking',
          amount: grossReward.toString(),
          feeAmount: platformFee.toString(),
          netAmount: netReward.toString(),
          currency: 'ETH',
          description: `Staking performance fee from pool ${pool.name}`,
          metadata: {
            poolId: pool.id,
            stakeId: stake.id,
            feePercentage: REVENUE_CONFIG.stakingPoolFeePercentage
          }
        });
        ...
        logRevenueEvent({
          ...
          netAmount: netReward,
          wallet: getRevenueWallet('staking'),
          timestamp: new Date(),
          metadata: { poolId: pool.id, stakeId: stake.id }
        });
      } catch (err) {
        console.error('Failed to record staking revenue:', err);
      }
    }

    console.log(`  💰 Stake ${stake.id}: ${currentBalance.toFixed(6)} → ${newBalance.toFixed(6)} ETH (+${netReward.toFixed(6)} net, ${platformFee.toFixed(6)} fee)`);
  }
