...
class FallbackNFTProvider implements NFTProvider {
  ...
  isAvailable(): boolean {
    return true; // Available as graceful fallback when external APIs are missing
  }

  async fetchNFTsByOwner(walletAddress: string, chainId: string, cursor?: string): Promise<NFTProviderResponse> {
    // Gracefully return empty NFTs instead of throwing error
    // This prevents app crashes when API keys are not configured
    console.warn('⚠️ NFT provider not configured. NFT features will be unavailable. To enable NFTs, set MORALIS_API_KEY or OPENSEA_API_KEY in Replit Secrets.');
  ...
  async fetchNFTMetadata(contractAddress: string, tokenId: string, chainId: string): Promise<NFTMetadata | null> {
    // Gracefully return null instead of throwing error
    // This allows the app to continue functioning without external NFT API
    console.warn('⚠️ NFT provider not configured. NFT metadata unavailable. To enable, set MORALIS_API_KEY or OPENSEA_API_KEY in Replit Secrets.');
