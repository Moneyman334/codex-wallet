import { 
  ...
  type InsertPageView,
  type VisitorEvent,
  type InsertVisitorEvent,
  billionaireProfiles,
  billionaireProjections,
  billionaireMilestones,
  type BillionaireProfile,
  type InsertBillionaireProfile,
  type BillionaireProjection,
  type InsertBillionaireProjection,
  type BillionaireMilestone,
  type InsertBillionaireMilestone,
  platformRevenue
} from "@shared/schema";
import { randomUUID } from "crypto";
import { drizzle } from "drizzle-orm/neon-http";
import { neon } from "@neondatabase/serverless";
import { eq, or, and, desc, sql, gte, lte } from "drizzle-orm";
...
export interface IStorage {
  ...
  getUnclaimedProfits(userId: string): Promise<{ totalProfit: number; tradeCount: number }>;
  claimBotProfits(userId: string): Promise<{ claimedAmount: number; tradeCount: number }>;
  ...
  getPlatformRevenueBySource(source: string, startDate?: Date, endDate?: Date): Promise<number>;
  getTotalPlatformRevenue(startDate?: Date, endDate?: Date): Promise<number>;
  ...
  clearPendingTransactions(): Promise<number>;
  ...
  getPageViews(sessionId?: string, path?: string): Promise<PageView[]>;
  createVisitorEvent(event: InsertVisitorEvent): Promise<VisitorEvent>;
  getVisitorEvents(sessionId?: string, eventType?: string): Promise<VisitorEvent[]>;
  ...
  // Billionaire Mode methods
  createBillionaireProfile(profile: InsertBillionaireProfile): Promise<BillionaireProfile>;
  getBillionaireProfile(userId: string): Promise<BillionaireProfile | undefined>;
  getBillionaireProfileByWallet(walletAddress: string): Promise<BillionaireProfile | undefined>;
  updateBillionaireProfile(userId: string, updates: Partial<InsertBillionaireProfile>): Promise<BillionaireProfile | undefined>;
  createBillionaireProjection(projection: InsertBillionaireProjection): Promise<BillionaireProjection>;
  getBillionaireProjections(profileId: string): Promise<BillionaireProjection[]>;
  createBillionaireMilestone(milestone: InsertBillionaireMilestone): Promise<BillionaireMilestone>;
  getBillionaireMilestones(profileId: string): Promise<BillionaireMilestone[]>;
  updateBillionaireMilestone(id: string, updates: Partial<InsertBillionaireMilestone>): Promise<BillionaireMilestone | undefined>;
}
...
export class MemStorage implements IStorage {
  ...
  async deleteUser(userId: string): Promise<boolean> {
    // Cascade delete ALL user-related data (Google Play compliance requirement)
    
    // Step 1: Get user's wallet addresses (needed for cascade delete)
    const userWallets = Array.from(this.wallets.values()).filter(w => w.userId === userId);
    const walletAddresses = userWallets.map(w => w.address.toLowerCase());
    
    // Step 2: Delete wallet-related data
    walletAddresses.forEach(address => {
      // Delete token balances
      Array.from(this.tokenBalances.entries()).forEach(([id, balance]) => {
        if (balance.walletAddress.toLowerCase() === address) {
          this.tokenBalances.delete(id);
        }
      });
      
      // Delete trusted addresses
      Array.from(this.trustedAddresses.entries()).forEach(([id, ta]) => {
        if (ta.walletAddress.toLowerCase() === address) {
          this.trustedAddresses.delete(id);
        }
      });
      
      // Delete blocked addresses
      Array.from(this.blockedAddresses.entries()).forEach(([id, ba]) => {
        if (ba.walletAddress.toLowerCase() === address) {
          this.blockedAddresses.delete(id);
        }
      });
      
      // Delete security alerts
      Array.from(this.securityAlerts.entries()).forEach(([id, alert]) => {
        if (alert.walletAddress.toLowerCase() === address) {
          this.securityAlerts.delete(id);
        }
      });
      
      // Delete transactions where wallet is from or to
      Array.from(this.transactions.entries()).forEach(([id, tx]) => {
        if (tx.fromAddress.toLowerCase() === address || tx.toAddress.toLowerCase() === address) {
          this.transactions.delete(id);
        }
      });
    });
    
    // Step 3: Delete user-scoped data (by userId)
    Array.from(this.userTokens.entries()).forEach(([id, ut]) => {
      if (ut.userId === userId) {
        this.userTokens.delete(id);
      }
    });
    
    Array.from(this.contracts.entries()).forEach(([id, contract]) => {
      if (contract.userId === userId) {
        this.contracts.delete(id);
        
        // Delete contract calls for this contract
        Array.from(this.contractCalls.entries()).forEach(([callId, call]) => {
          if (call.contractId === id) {
            this.contractCalls.delete(callId);
          }
        });
        
        // Delete contract event subscriptions for this contract
        Array.from(this.contractEventSubs.entries()).forEach(([subId, sub]) => {
          if (sub.contractId === id) {
            this.contractEventSubs.delete(subId);
          }
        });
      }
    });
    
    // Step 4: Delete user's wallets
    userWallets.forEach(wallet => this.wallets.delete(wallet.id));
  ...
  async clearPendingTransactions() { return 0; }
  ...
  getUnclaimedProfits = undefined as any;
  claimBotProfits = undefined as any;
...
export class PostgreSQLStorage implements IStorage {
  ...
  async getUnclaimedProfits(userId: string) {
    ...
    .where(
      and(
        ...
        eq(botTrades.profitClaimed, 'false'),
    ...
    return {
      ...
      tradeCount: parseInt(result[0]?.tradeCount || '0', 10)
    };
  }

  async claimBotProfits(userId: string) {
    // Use UPDATE ... RETURNING to atomically claim profits and get results in one transaction
    const claimedTrades = await db.update(botTrades)
      .set({ 
        profitClaimed: 'true',
        claimedAt: new Date()
      })
    ...
    const claimedAmount = claimedTrades.reduce((sum, trade) => 
      sum + parseFloat(trade.profit || '0'), 0
    );

    return {
      claimedAmount,
      tradeCount: claimedTrades.length
    };
  }
  ...
  async recordPlatformRevenue(data: {
    ...
    try {
      const { REVENUE_CONFIG, getRevenueWallet } = await import('./revenue-config');
      const walletAddress = getRevenueWallet(data.source as any) || REVENUE_CONFIG.dexFeeWallet;
      ...
      await db.insert(platformRevenue).values({
        revenueType: data.source,
        amount: data.amount,
        feeAmount: data.feeAmount || data.amount,
      ...
      console.log(`💰 Platform revenue recorded: ${data.amount} ${data.currency} from ${data.source}`);
    } catch (error) {
      console.error('Failed to record platform revenue:', error);
    }
  }
  
  async getPlatformRevenueBySource(source: string, startDate?: Date, endDate?: Date) {
    let query = db.select({
      total: sql<string>`COALESCE(SUM(CAST(${platformRevenue.feeAmount} AS NUMERIC)), 0)`
    }).from(platformRevenue).where(eq(platformRevenue.revenueType, source));
    
    if (startDate && endDate) {
      query = db.select({
        total: sql<string>`COALESCE(SUM(CAST(${platformRevenue.feeAmount} AS NUMERIC)), 0)`
      }).from(platformRevenue).where(
        and(
          eq(platformRevenue.revenueType, source),
  ...
  async getTotalPlatformRevenue(startDate?: Date, endDate?: Date) {
    let query;
  ...
  async createVisitorSession(session: InsertVisitorSession) {
    const [created] = await db.insert(visitorSessions)
      .values(session)
      .returning();
    return created;
  }
  ...
  async updateVisitorSession(sessionId: string, updates: Partial<InsertVisitorSession>) {
  ...
  async createPageView(pageView: InsertPageView) {
    const [created] = await db.insert(pageViews)
      .values(pageView)
      .returning();
    return created;
  }
  
  async getPageViews(sessionId?: string, path?: string) {
    let query = db.select().from(pageViews);
    
    if (sessionId && path) {
      query = query.where(and(eq(pageViews.sessionId, sessionId), eq(pageViews.path, path))) as any;
    } else if (sessionId) {
      query = query.where(eq(pageViews.sessionId, sessionId)) as any;
    } else if (path) {
      query = query.where(eq(pageViews.path, path)) as any;
    }
    
    return await query.orderBy(desc(pageViews.viewedAt));
  }
  
  async createVisitorEvent(event: InsertVisitorEvent) {
    const [created] = await db.insert(visitorEvents)
      .values(event)
      .returning();
    return created;
  }
  
  async getVisitorEvents(sessionId?: string, eventType?: string) {
    let query = db.select().from(visitorEvents);
    
    if (sessionId && eventType) {
      query = query.where(and(eq(visitorEvents.sessionId, sessionId), eq(visitorEvents.eventType, eventType))) as any;
    } else if (sessionId) {
      query = query.where(eq(visitorEvents.sessionId, sessionId)) as any;
    } else if (eventType) {
      query = query.where(eq(visitorEvents.eventType, eventType)) as any;
    }
    
    return await query.orderBy(desc(visitorEvents.occurredAt));
  }
  
  async getVisitorStats(startDate?: Date, endDate?: Date) {
    let sessionQuery = db.select().from(visitorSessions);
    
    if (startDate && endDate) {
      sessionQuery = sessionQuery.where(
        and(
          sql`${visitorSessions.startedAt} >= ${startDate}`,
          sql`${visitorSessions.startedAt} <= ${endDate}`
        )
      ) as any;
    } else if (startDate) {
      sessionQuery = sessionQuery.where(sql`${visitorSessions.startedAt} >= ${startDate}`) as any;
    } else if (endDate) {
      sessionQuery = sessionQuery.where(sql`${visitorSessions.startedAt} <= ${endDate}`) as any;
    }
    ...
    const avgSessionDuration = sessions.reduce((sum, s) => sum + parseInt(s.duration || "0"), 0) / (totalSessions || 1);
    
    // Get page view stats
    let pageViewQuery = db.select({
      path: pageViews.path,
      count: sql<number>`count(*)::int`
    }).from(pageViews).groupBy(pageViews.path).orderBy(desc(sql`count(*)`)).limit(10);
    
    if (startDate && endDate) {
      pageViewQuery = pageViewQuery.where(
        and(
          sql`${pageViews.viewedAt} >= ${startDate}`,
          sql`${pageViews.viewedAt} <= ${endDate}`
        )
      ) as any;
    }
    
    const topPagesResult = await pageViewQuery;
    const topPages = topPagesResult.map(p => ({ path: p.path, views: p.count }));
    
    // Get total page views count
    const [totalPageViewsResult] = await db.select({ count: sql<number>`count(*)::int` }).from(pageViews);
    const totalPageViews = totalPageViewsResult?.count || 0;
    
    // Device breakdown
    const deviceBreakdownResult = await db.select({
      device: visitorSessions.device,
      count: sql<number>`count(*)::int`
    }).from(visitorSessions).groupBy(visitorSessions.device);
    
    const deviceBreakdown = deviceBreakdownResult
      .filter(d => d.device)
      .map(d => ({ device: d.device!, count: d.count }));
    
    // Browser breakdown
    const browserBreakdownResult = await db.select({
      browser: visitorSessions.browser,
      count: sql<number>`count(*)::int`
    }).from(visitorSessions).groupBy(visitorSessions.browser);
    
    const browserBreakdown = browserBreakdownResult
      .filter(b => b.browser)
      .map(b => ({ browser: b.browser!, count: b.count }));
  ...
  async createBillionaireProfile(profile: InsertBillionaireProfile) {
    const [created] = await db.insert(billionaireProfiles)
      .values(profile)
      .returning();
    return created;
  }
  
  async getBillionaireProfile(userId: string) {
    const [profile] = await db.select().from(billionaireProfiles)
      .where(eq(billionaireProfiles.userId, userId));
    return profile;
  }
  
  async getBillionaireProfileByWallet(walletAddress: string) {
    const [profile] = await db.select().from(billionaireProfiles)
      .where(eq(billionaireProfiles.walletAddress, walletAddress));
    return profile;
  }
  
  async updateBillionaireProfile(userId: string, updates: Partial<InsertBillionaireProfile>) {
    const [updated] = await db.update(billionaireProfiles)
      .set({ ...updates, lastUpdated: new Date() })
      .where(eq(billionaireProfiles.userId, userId))
      .returning();
    return updated;
  ...
  async createBillionaireProjection(projection: InsertBillionaireProjection) {
    const [created] = await db.insert(billionaireProjections)
      .values(projection)
      .returning();
    return created;
  }
  
  async getBillionaireProjections(profileId: string) {
    return await db.select().from(billionaireProjections)
      .where(eq(billionaireProjections.profileId, profileId))
      .orderBy(billionaireProjections.age);
  }
  
  async createBillionaireMilestone(milestone: InsertBillionaireMilestone) {
    const [created] = await db.insert(billionaireMilestones)
      .values(milestone)
      .returning();
    return created;
  }
  
  async getBillionaireMilestones(profileId: string) {
    return await db.select().from(billionaireMilestones)
      .where(eq(billionaireMilestones.profileId, profileId))
      .orderBy(billionaireMilestones.createdAt);
  }
  
  async updateBillionaireMilestone(id: string, updates: Partial<InsertBillionaireMilestone>) {
    const [updated] = await db.update(billionaireMilestones)
      .set(updates)
      .where(eq(billionaireMilestones.id, id))
      .returning();
    return updated;
  }
