...
export class DexAggregator {
  ...
  async getSwapTransaction(params: {
    ...
    // Graceful fallback if API key is missing - return error that user can recover from
    if (!this.apiKey) {
      console.warn('⚠️ 1inch API key not configured. DEX swap disabled. To enable, set ONEINCH_API_KEY in Replit Secrets.');
      throw new Error('DEX trading unavailable: 1inch API key not configured. Contact admin to enable trading.');
    }

    // Platform fee wallet address (where 0.3% fee goes)
    // Import from centralized revenue config
    const { getPlatformFeeWallet, REVENUE_CONFIG } = await import('./revenue-config');
    const FEE_WALLET = getPlatformFeeWallet();
    const PLATFORM_FEE_BPS = REVENUE_CONFIG.dexFeeBasisPoints; // 0.3% in basis points (30/10000 = 0.003)
  ...
  private async getFallbackQuote(params: {
    ...
    try {
      ...
      if (!fromPrice || !toPrice) {
        console.warn(`⚠️ Unable to fetch prices for ${fromCoinId} or ${toCoinId}`);
      ...
      console.error('⚠️ Fallback quote calculation failed:', error);
      throw new Error('Unable to calculate fallback quote. Please try again later.');
    }
  }
